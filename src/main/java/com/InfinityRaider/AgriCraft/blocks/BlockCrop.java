 ...
 */
@Optional.InterfaceList(value = {@Optional.Interface(modid = Names.Mods.botania, iface = "vazkii.botania.api.item.IHornHarvestable"), @Optional.Interface(modid = Names.Mods.ancientWarfare, iface = "net.shadowmage.ancientwarfare.api.IAncientWarfareFarmable"), @Optional.Interface(modid = Names.Mods.mfr, iface = "powercrystals.minefactoryreloaded.api.IFactoryFertilizable")})
        ...
        if (crop.hasPlant() || crop.hasWeed()) {
            ...
            if (allowGrowthResult != Event.Result.DENY) {
                if (crop.isMature() && crop.hasWeed() && ConfigurationHandler.enableWeeds) {
                    ...
                    double growthBonus = 1.0 + Math.min((double) crop.getGrowth() / 10.0, 10.0);
                    ...
                    if (shouldGrow) {
                        crop.applyGrowthTick(world, x, y, z, rnd);
                    }
                }
            }
        } else {
    ...
    public boolean plantSeed(ItemStack stack, World world, int x, int y, int z) {
        if (!world.isRemote) {
            ...
            if (stack.stackTagCompound != null && stack.stackTagCompound.hasKey(Names.NBT.growth)) {
                ...
                if (stack.stackTagCompound != null && stack.stackTagCompound.hasKey("growth")) {
                    if (ConfigurationHandler.enableBetaSys && stack.stackTagCompound.hasKey("DIMadaptability")) {
                        crop.setPlant(stack.stackTagCompound.getInteger("growth"), stack.stackTagCompound.getInteger("gain"), stack.stackTagCompound.getInteger("strength"), stack.stackTagCompound.getBoolean("analyzed"), CropPlantHandler.getPlantFromStack(new ItemStack(stack.getItem(), 1, stack.getItemDamage())), stack.stackTagCompound.getInteger("DIMadaptability"));
                    } else {
                        crop.setPlant(stack.stackTagCompound.getInteger("growth"), stack.stackTagCompound.getInteger("gain"), stack.stackTagCompound.getInteger("strength"), stack.stackTagCompound.getBoolean("analyzed"), stack.getItem(), stack.getItemDamage());
                    }
                } else {
